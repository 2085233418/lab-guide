...
完成本节任务（Tasks 11-14）后，可以触发终端可视化来查看进程上下文的保存效果。

### 如何触发可视化

在 [os/src/main.rs](../第六章实验/os/src/main.rs) 文件中，找到 `test_process_management_progressive()` 函数，**取消第124行的注释**：

```rust
// ========================================
// Module 3: Process Context (Tasks 11-14)
// Uncomment after completing Task 14
// ========================================
// UNCOMMENT_AFTER_TASK_14
test_process_context();  // ← 删除这行前面的 //
```
...
取消注释后，系统会自动调用 `ProcessContext::visualize()` 方法显示：

```
[Module 3] Process Context (Tasks 11-14)
----------------------------------------
RA: 0x80200000 | SP: 0x80300000

========================================
  Process Context: TestProcess
========================================
...
| 字段 | 含义 | 说明 |
|------|------|------|
| **Process Context** | 进程名称 | 当前展示的进程上下文所属进程 |
| **Program Counter (sepc)** | 程序计数器 | 进程下次执行的指令地址 |
| **Status Register (sstatus)** | 状态寄存器 | 处理器状态（特权级、中断使能等） |
| **Page Table (satp)** | 页表基址 | 进程的虚拟内存页表位置 |
| **ra (x1)** | 返回地址寄存器 | 函数调用的返回地址 |
| **sp (x2)** | 栈指针寄存器 | 当前栈顶位置 |
| **gp (x3)** | 全局指针寄存器 | 全局数据区指针 |
| **tp (x4)** | 线程指针寄存器 | 线程局部存储指针 |

### 为什么需要保存这些寄存器？

当操作系统进行进程切换时，必须保存当前进程的所有寄存器状态，否则：

- **丢失PC**: 不知道进程执行到哪里，无法恢复执行
- **丢失SP**: 栈指针错误，函数调用会崩溃
- **丢失RA**: 函数无法正确返回
- **丢失通用寄存器**: 进程的计算结果全部丢失

这就像你做作业被打断，如果不记录做到第几题，回来后就不知道从哪里继续了。

**可视化代码位置**: [os/src/process/context.rs:121](../第六章实验/os/src/process/context.rs#L121)
