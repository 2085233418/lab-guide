...
![P1](/Users/weisiyang/操作系统能力大赛/lab/project3035746-355951/chapters/chapter7/pictures/p1.png)
...
现在轮到你动手了！请打开 `os/src/fs/inode.rs` 文件，完成以下3个任务点：

> **Task 2-1：定义用户权限位(3)**
>
> 在 `permissions` 模块中定义用户（owner）的读、写、执行权限常量：
> - `S_IRUSR`：用户读权限 = `0o400`
> - `S_IWUSR`：用户写权限 = `0o200`
> - `S_IXUSR`：用户执行权限 = `0o100`

> **Task 2-2：定义组权限位（3分**
>
> 定义组（group）的读、写、执行权限常量：
> - `S_IRGRP`：组读权限 = `0o040`
> - `S_IWGRP`：组写权限 = `0o020`
> - `S_IXGRP`：组执行权限 = `0o010`

> **Task 2-3：定义其他用户权限位（3分**
>
> 定义其他用户（others）的读、写、执行权限常量：
> - `S_IROTH`：其他读权限 = `0o004`
> - `S_IWOTH`：其他写权限 = `0o002`
> - `S_IXOTH`：其他执行权限 = `0o001`

**代码位置**：`os/src/fs/inode.rs` 中的 `permissions` 模块
...
/// Unix权限系统的核心是这9个位的组合
pub mod permissions {
    // TODO: Task 2-1 - 定义用户（文件所有者）权限位（3分）
    // Hint: 用户权限占最高3位，格式 0o4XX
    //示例
    ...
    // TODO: Task 2-2 - 定义组（文件所属组）权限位（3分）
    // Hint: 组权限占中间3位，格式 0o0XX

    // TODO: Task 2-3 - 定义其他用户权限位（3分）
    // Hint: 其他权限占最低3位，格式 0o00X
...
###  学生任务：实现 Inode trait 的方法

现在请打开 `os/src/fs/inode.rs` 文件，为 MemInode 实现 Inode trait 的4个方法：

> **Task 2-4：实现 ino() 方法（2分）**
>
> 返回 Inode 编号（`self.ino`）

> **Task 2-5：实现 file_type() 方法（2分）**
>
> 返回文件类型（`self.file_type`）

> **Task 2-6：实现 size() 方法（2分）**
>
> 返回文件大小（`self.size`）

> **Task 2-7：实现 mode() 方法（2分）**
>
> 返回权限位（`self.mode`）

**代码位置**：`os/src/fs/inode.rs` 中的 `impl Inode for MemInode` 块

**参考代码框架**：

```rust
impl Inode for MemInode {
    // TODO: Task 2-4 - 实现ino方法 (2 points)
    // Hint: 返回self.ino


    // TODO: Task 2-5 - 实现file_type方法 (2 points)
    // Hint: 返回self.file_type

    // TODO: Task 2-6 - 实现size方法 (2 points)
    // Hint: 返回self.size

    // TODO: Task 2-7 - 实现mode方法 (2 points)
    // Hint: 返回self.mode
}
```

完成上面任务后请思考**这个实现很简单，为什么还要特地设置？**

```rust
// 通过Inode trait，可以统一处理不同类型的Inode
