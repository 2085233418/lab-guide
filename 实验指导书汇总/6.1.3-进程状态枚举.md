...
完成本节任务（Tasks 7-10）后，可以触发终端可视化来查看进程状态转换效果。

### 如何触发可视化

在 [os/src/main.rs](../第六章实验/os/src/main.rs) 文件中，找到 `test_process_management_progressive()` 函数，**取消第117行的注释**：

```rust
// ========================================
// Module 2: Process State (Tasks 7-10)
// Uncomment after completing Task 10
// ========================================
// UNCOMMENT_AFTER_TASK_10
test_process_state();  // ← 删除这行前面的 //
```
...
取消注释后，系统会自动调用 `ProcessState::visualize_transition()` 函数显示：

```
[Module 2] Process State (Tasks 7-10)
----------------------------------------
States: Ready | Running
Ready can schedule: true
Ready -> Running: true
...
| 字段 | 含义 | 说明 |
|------|------|------|
| **From** | 起始状态 | 进程转换前的状态（Ready/Running/Blocked/Zombie） |
| **To** | 目标状态 | 进程要转换到的状态 |
| **Valid** | 是否合法 | true表示允许转换，false表示非法转换 |
| **结果标识** | 转换结果 | `+ Transition allowed` 或 `x Invalid transition!` |

### 合法的状态转换

系统会展示以下合法转换：
- **Ready → Running**: 进程被调度器选中，获得CPU执行权
- **Running → Ready**: 时间片用完，进程回到就绪队列
- **Running → Blocked**: 进程等待I/O或其他事件
- **Running → Zombie**: 进程执行完毕退出
- **Blocked → Ready**: I/O完成，进程重新进入就绪队列

### 非法的状态转换

如果尝试非法转换（如 Ready → Zombie），输出会显示 `x Invalid transition!`，表示这是不允许的状态转换。

**可视化代码位置**: [os/src/process/pcb.rs:115](../第六章实验/os/src/process/pcb.rs#L115)
