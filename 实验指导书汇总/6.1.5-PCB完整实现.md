...
完成本节任务（Tasks 15-19）后，可以触发终端可视化来查看PCB（进程控制块）的详细信息。

### 如何触发可视化

在 [os/src/main.rs](../第六章实验/os/src/main.rs) 文件中，找到 `test_process_management_progressive()` 函数，**取消第131行的注释**：

```rust
// ========================================
// Module 4: PCB (Tasks 15-19)
// Uncomment after completing Task 19
// ========================================
// UNCOMMENT_AFTER_TASK_19
test_process_control_block();  // ← 删除这行前面的 //
```
...
**⚠️ 重要提示**：如果你还没有完成Tasks 15-19的所有填空，取消注释后运行会出现编译错误或panic。请确保完成所有PCB相关的实现后再取消注释。

### 可视化输出示例1：单个PCB详情

取消注释后，系统会自动调用 `pcb.visualize()` 方法显示单个进程的完整信息：

```
[Module 4] Process Control Block (Tasks 15-19)
----------------------------------------
PCB: init (1)
Children: 1
...
========================================
Program Counter (sepc):  0x0
Status Register (sstatus): 0x0
Page Table (satp):       0x0
...
系统还会自动调用 `ProcessControlBlock::visualize_process_list()` 显示所有进程：

```
========================================
  Process List (Real System State)
========================================
 PID | Name         | State    | Parent | Children
-----|--------------|----------|--------|----------
   1 | init         | Running  |   -    | 1
   2 | shell        | Ready    |      1 | 0
========================================
...
| 字段 | 含义 | 说明 |
|------|------|------|
| **PID** | 进程标识符 | 系统中唯一的进程编号 |
| **Name** | 进程名称 | 进程的可读名称 |
| **State** | 当前状态 | Ready/Running/Blocked/Zombie |
| **Parent PID** | 父进程ID | 创建该进程的父进程，init进程显示None |
| **Children** | 子进程数量 | 该进程创建的子进程个数 |
| **Priority** | 优先级 | 调度优先级（数字越小优先级越高） |
| **Time Slice** | 时间片 | 进程可连续执行的时钟周期数 |
| **Exit Code** | 退出码 | 进程退出时的返回值（仅Zombie状态显示） |

**进程列表字段说明**：

- **PID**: 进程ID号
- **Name**: 进程名称（最多12个字符）
- **State**: 当前状态（Running/Ready/Blocked/Zombie）
- **Parent**: 父进程PID，根进程显示"-"
- **Children**: 子进程数量
...
- `pcb.visualize()` - [os/src/process/pcb.rs:375](../第六章实验/os/src/process/pcb.rs#L375)
- `ProcessControlBlock::visualize_process_list()` - [os/src/process/pcb.rs:405](../第六章实验/os/src/process/pcb.rs#L405)
