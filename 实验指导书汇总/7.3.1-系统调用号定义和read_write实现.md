...
```
用户态:
  read(fd=3, buf=0x1000, len=100)
    ↓ ecall指令
    ↓ 陷入内核（U模式 → S模式）
内核态:
  sys_read(3, 0x1000, 100):
    1. FD_TABLE.get(3) → 找到File对象
    2. 分配内核缓冲区 kernel_buf[100]
    3. file.read(&mut kernel_buf) → 从文件读取到内核缓冲区
    4. copy_from_slice → 从内核缓冲区拷贝到用户缓冲区(0x1000)
    5. 返回实际读取字节数: 50
    ↓ sret指令
用户态:
  返回值 = 50 (存储在寄存器a0)
